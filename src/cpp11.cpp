// Generated by cpp11: do not edit by hand
// clang-format off


#include "cpp11/declarations.hpp"
#include <R_ext/Visibility.h>

// gridburn.cpp
cpp11::writable::list cpp_burn_sparse(cpp11::list wkb_list, double xmin, double ymin, double xmax, double ymax, int ncol, int nrow);
extern "C" SEXP _gridburn_cpp_burn_sparse(SEXP wkb_list, SEXP xmin, SEXP ymin, SEXP xmax, SEXP ymax, SEXP ncol, SEXP nrow) {
  BEGIN_CPP11
    return cpp11::as_sexp(cpp_burn_sparse(cpp11::as_cpp<cpp11::decay_t<cpp11::list>>(wkb_list), cpp11::as_cpp<cpp11::decay_t<double>>(xmin), cpp11::as_cpp<cpp11::decay_t<double>>(ymin), cpp11::as_cpp<cpp11::decay_t<double>>(xmax), cpp11::as_cpp<cpp11::decay_t<double>>(ymax), cpp11::as_cpp<cpp11::decay_t<int>>(ncol), cpp11::as_cpp<cpp11::decay_t<int>>(nrow)));
  END_CPP11
}
// gridburn_init.cpp
void cpp_gridburn_init();
extern "C" SEXP _gridburn_cpp_gridburn_init() {
  BEGIN_CPP11
    cpp_gridburn_init();
    return R_NilValue;
  END_CPP11
}

extern "C" {
static const R_CallMethodDef CallEntries[] = {
    {"_gridburn_cpp_burn_sparse",   (DL_FUNC) &_gridburn_cpp_burn_sparse,   7},
    {"_gridburn_cpp_gridburn_init", (DL_FUNC) &_gridburn_cpp_gridburn_init, 0},
    {NULL, NULL, 0}
};
}

extern "C" attribute_visible void R_init_gridburn(DllInfo* dll){
  R_registerRoutines(dll, NULL, CallEntries, NULL, NULL);
  R_useDynamicSymbols(dll, FALSE);
  R_forceSymbols(dll, TRUE);
}
